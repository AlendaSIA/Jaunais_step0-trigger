...
def run(ctx: dict):
    ctx["step_04_version"] = "v2026-03-11-01"
    ...
    if gh_token:
        ...
        st1, sn1 = _github_put_file(
            gh_token,
            json_path,
            pretty.encode("utf-8"),
            f"debug: extract_all json {doc_id}",
        )
        st2, sn2 = _github_put_file(
            gh_token,
            html_path,
            html.encode("utf-8"),
            f"debug: extract_all html {doc_id}",
        )
